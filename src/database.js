...
// Only JWT_SECRET is required to crash. Others are derived.
var JWT_SECRET = process.env.JWT_SECRET || “”;
if (!JWT_SECRET || JWT_SECRET.length < 32) {
console.error(”[FATAL] JWT_SECRET is missing or too short (min 32 chars).”);
process.exit(1);
}
// Derive missing secrets from JWT_SECRET so existing deployments keep working
...
// Suppress WAL ghost errors from previous bad deploys
...
// Kill any leftover rebuild table from a crashed previous deploy
await execSQL(“DROP TABLE IF EXISTS _users_old_rebuild”);

// Rebuild users table if password column has NOT NULL (old schema)
