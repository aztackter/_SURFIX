...
const LUA_B64_DEC = `local function _SFB64(s) local A="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/" local t={} for i=1,#A do t[A:sub(i,i)]=i-1 end local o,n,p="",0,0 for i=1,#s do local c=t[s:sub(i,i)] if c then n=n*64+c p=p+6 if p>=8 then p=p-8 o=o..string.char(math.floor(n/2^p)%256) end end end return o end`;

const LUA_XOR_DEC = `local function _SFXD(d,k) local o={} local l=#k for i=1,#d do o[i]=string.char(d:byte(i)~k:byte((i-1)%l+1)) end return table.concat(o) end`;

const LUA_FNV = `local function _SFFNV(s) local h=2166136261 for i=1,#s do h=(h~s:byte(i))*16777619 h=h%4294967296 end return string.format("%08x",h) end`;
...
const vm = ` local function ${vmId}(bc,kp) local _ip=1 local function _rb() local b=bc:byte(_ip) _ip=_ip+1 return b end local _acc=nil while _ip<=#bc do local op=_rb() if op==${OP_LOADK} then local idx=_rb() _acc=kp[idx] elseif op==${OP_CHK} then if _SFFNV(_acc):sub(1,8)~="${checksum.slice(0,8)}" then error("[SURFIX] Integrity violation") end elseif op==${OP_EXEC} then local fn,e=loadstring(_acc) if not fn then error("[SURFIX] "..tostring(e)) end fn() elseif op==${OP_END} then break end end end`;
...
if (lightning) return “”;
return ` local function _SFADB() local ok=pcall(function() if debug and debug.sethook then local _t=0 debug.sethook(function() _t=_t+1 end,"l") local _a=0 for i=1,10 do _a=_a+i end debug.sethook() if _t>0 then error("D") end end end) if not ok then return end if debug and debug.getinfo then local i=debug.getinfo(1) if i and i.what=="C" then error("[SURFIX] Debugger detected") end end end pcall(_SFADB)`;
}
